...
def generate_cvs_cluster(data, amount_of_clusters):
    ...
    # reviews = [f"{review['review']} &{review['score']}" for review in data.values()]
    ...
    # Make a text file for each cluster named cluster_1.txt, cluster_2.txt, etc. Each file should first contain the
    # cluster number, the amount of reviews and then the reviews in that cluster.
    for i in range(amount_of_clusters):
        with open(f"cvs_clusters/cluster_{i}_{len(df[df['cluster'] == i])}.txt", "w") as my_file:
            ...
            for review in df[df['cluster'] == i]['review']:
                # split the review and score if multiple &'s are detected, use only the last one
                # review, score = review.rsplit("&", 1)
                # write the review and score to the file
                my_file.write(f"{review}\n\n")
                # my_file.write(f"Score: {score}\n\n")
    # end the timer
...
if __name__ == '__main__':
    ...
    generate_cvs_cluster(dicti, 100)
